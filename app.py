...
class LoanEligibilityApp:
    ...
    def setup_app(self):
        ...
        self.app.config['DATABASE_PATH'] = os.environ.get('DATABASE_PATH', 'loan_prediction.db')
    ...
    def load_ml_models(self):
        ...
        try:
            model_path = os.path.join(self.app.config['MODEL_PATH'], 'loan_eligibility_model.pkl')
            scaler_path = os.path.join(self.app.config['MODEL_PATH'], 'feature_scaler.pkl')
            ...
            if os.path.exists(model_path) and os.path.exists(scaler_path):
                ...
                logger.warning("ML model files not found. Prediction service will be unavailable.")
    ...
    def register_routes(self):
        ...
        def user_dashboard():
            ...
            return render_template('dashboard/home.html', user_name=session.get('full_name'))
        ...
        def predict_loan_eligibility():
            ...
            if not self.prediction_model or not self.feature_scaler:
                return jsonify({'success': False, 'message': 'Prediction service unavailable'}), 503
            ...
            try:
                ...
                self.save_prediction_record(session['user_id'], form_data, prediction_result)
    ...
    def extract_loan_features(self, form_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def generate_prediction(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        loan_to_income_ratio = features['loan_amount'] / total_income if total_income > 0 else 0
        emi_feature = features['loan_amount'] / features['loan_tenure'] if features['loan_tenure'] > 0 else 0
        ...
        feature_vector = pd.DataFrame({
            ...
            'Education_Not Graduate': [1 if features['education_level'] == 1 else 0],
            'Self_Employed_Yes': [1 if features['employment_type'] == 1 else 0],
            'Property_Area_Semiurban': [1 if features['property_location'] == 1 else 0],
    ...
    def save_prediction_record(self, user_id: str, form_data: Dict[str, Any], prediction: Dict[str, Any]):
        ...
        try:
            ...
            with self.get_database_connection() as conn:
                ...
                ''', (
                    user_id, features['first_name'], features['last_name'],
                    features['gender'], features['marital_status'], features['dependents_count'],
                    features['education_level'], features['employment_type'], features['property_location'],
                    features['credit_history'], features['cibil_score'], features['applicant_income'],
                    features['coapplicant_income'], features['loan_amount'], features['loan_tenure'],
